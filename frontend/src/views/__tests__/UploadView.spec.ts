...
describe('UploadView', () => {
  ...
  describe('upload flow', () => {
    it('uploads files via pre-signed URL and shows Uploaded status with reference IDs', async () => {
      ...
      vi.spyOn(globalThis, 'fetch')
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ uploadUrl: 'https://s3.example.com/upload', referenceId: 'REF-001' }), { status: 200 })
        )
        .mockResolvedValueOnce(
    ...
    it('sends correct presign request body', async () => {
      ...
      const fetchSpy = vi.spyOn(globalThis, 'fetch')
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ uploadUrl: 'https://s3.example.com/upload', referenceId: 'REF-002' }), { status: 200 })
        )
  ...
  describe('upload error handling', () => {
    ...
    it('shows error status when S3 upload fails', async () => {
      ...
      vi.spyOn(globalThis, 'fetch')
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ uploadUrl: 'https://s3.example.com/upload', referenceId: 'REF-X' }), { status: 200 })
        )
        .mockResolvedValueOnce(
  ...
  describe('confirmation screen', () => {
    it('shows confirmation with all reference IDs after all files uploaded', async () => {
      ...
      vi.spyOn(globalThis, 'fetch')
        // File 1: presign + S3
        .mockResolvedValueOnce(new Response(JSON.stringify({ uploadUrl: 'https://s3.example.com/1', referenceId: 'REF-AAA' }), { status: 200 }))
        .mockResolvedValueOnce(new Response(null, { status: 200 }))
        // File 2: presign + S3
        .mockResolvedValueOnce(new Response(JSON.stringify({ uploadUrl: 'https://s3.example.com/2', referenceId: 'REF-BBB' }), { status: 200 }))
        .mockResolvedValueOnce(new Response(null, { status: 200 }))
    ...
    it('allows uploading more files after confirmation via reset button', async () => {
      ...
      vi.spyOn(globalThis, 'fetch')
        .mockResolvedValueOnce(new Response(JSON.stringify({ uploadUrl: 'https://s3.example.com/1', referenceId: 'REF-123' }), { status: 200 }))
        .mockResolvedValueOnce(new Response(null, { status: 200 }))
