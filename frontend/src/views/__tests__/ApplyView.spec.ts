...
describe('ApplyView', () => {
  ...
  describe('confirmation screen', () => {
    it('shows confirmation with reference ID on successful submit', async () => {
      ...
      vi.spyOn(globalThis, 'fetch').mockResolvedValue(
        new Response(JSON.stringify({ application_id: 'APP-ID-123', reference_number: '2025-0042' }), { status: 200 })
      )
      ...
      expect(wrapper.find('.apply-confirmation__ref').text()).toContain('2025-0042')
    })
