...
async function saveEdit() {
  ...
  try {
    const years = editForm.value.authorized_giveaway_years
      .split(',').map(s => s.trim()).filter(Boolean)
    const res = await fetch(`${API_BASE}/users/${editingUser.value.user_id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function userAction(userId: string, action: 'disable' | 'enable' | 'delete' | 'reset_password') {
  ...
  try {
    ...
    if (action === 'disable') {
      url = `${API_BASE}/users/${userId}/disable`; method = 'POST'
    } else if (action === 'enable') {
      url = `${API_BASE}/users/${userId}/enable`; method = 'POST'
    } else if (action === 'delete') {
      url = `${API_BASE}/users/${userId}`; method = 'DELETE'
    } else {
      url = `${API_BASE}/users/${userId}`; method = 'PUT'
...
.users-view h1 { margin: 0; font-size: 1.5rem; }
.users-view h2 { margin: 0 0 0.75rem; font-size: 1.1rem; }

.users-view__btn { padding: 0.4rem 1rem; border: 1px solid #4a90d9; background: #4a90d9; color: #fff; border-radius: 4px; font-size: 0.85rem; cursor: pointer; }
.users-view__btn:hover:not(:disabled) { background: #357abd; }
.users-view__btn:disabled { opacity: 0.6; cursor: not-allowed; }
.users-view__btn--secondary { background: #fff; color: #4a90d9; }

.users-view__form { padding: 1rem; background: #f9f9f9; border-radius: 6px; margin-bottom: 1rem; max-width: 500px; }
...
.users-view__input { padding: 0.35rem 0.5rem; border: 1px solid #ccc; border-radius: 4px; font-size: 0.85rem; width: 100%; box-sizing: border-box; }
.users-view__select { padding: 0.35rem 0.5rem; border: 1px solid #ccc; border-radius: 4px; font-size: 0.85rem; width: 100%; }
.users-view__actions { display: flex; gap: 0.5rem; margin-top: 0.5rem; }

.users-view__table { width: 100%; border-collapse: collapse; font-size: 0.85rem; }
.users-view__table th, .users-view__table td { padding: 0.5rem 0.6rem; text-align: left; border-bottom: 1px solid #e0e0e0; }
.users-view__table th { background: #f5f5f5; font-weight: 600; white-space: nowrap; }
...
.users-view__actions-cell { display: flex; gap: 0.4rem; flex-wrap: wrap; }
.users-view__link { background: none; border: none; color: #4a90d9; cursor: pointer; font-size: 0.8rem; padding: 0.15rem 0; }
...
.users-view__loading { padding: 1.5rem; text-align: center; color: #666; }
.users-view__empty { padding: 1.5rem; text-align: center; color: #666; }
