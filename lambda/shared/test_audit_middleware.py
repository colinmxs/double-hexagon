...
class TestLogAuditEvent:
    ...
    def test_generates_correct_year_month(self, patch_table):
        """year_month partition key should match the timestamp's YYYY-MM."""
        with patch("audit_middleware.datetime") as mock_dt:
            ...
            item = patch_table.put_item.call_args[1]["Item"]
            assert item["year_month"] == "2025-11"

    def test_generates_composite_sort_key(self, patch_table):
        """Sort key should be timestamp#user_id."""
        with patch("audit_middleware.datetime") as mock_dt:
            fixed = datetime(2025, 11, 15, 10, 30, 0, 0, tzinfo=timezone.utc)
            mock_dt.now.return_value = fixed
            mock_dt.side_effect = lambda *a, **kw: datetime(*a, **kw)
            ...
            item = patch_table.put_item.call_args[1]["Item"]
            assert item["timestamp#user_id"] == "2025-11-15T10:30:00.000Z#user-42"
...
class TestLogAuditFromContext:
    ...
    def test_handles_missing_keys_with_defaults(self, patch_table):
        ...
        result = log_audit_from_context({}, "login", "user_account", "u1")
