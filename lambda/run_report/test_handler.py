...
SAMPLE_APPS = [
    ...
    _make_application(
        "APP003", "needs_review", "digital", "Ana", "Lopez", "Agency A",
        children=[
...
class TestSummaryStatistics:
    ...
    def test_summary_by_status(self, mock_get_table, mock_auth):
        ...
        assert by_status["needs_review"] == 2
...
class TestFilterOperators:
    ...
    def test_in_list_filter(self, mock_get_table, mock_auth):
        ...
        event = _make_event({
            "giveaway_year": "2025",
            "columns": ["status"],
            "filters": [{"field": "status", "operator": "in_list", "value": ["manually_approved", "needs_review"]}],
        })
        result = handler(event, None)
        body = json.loads(result["body"])
    ...
    def test_multiple_filters_and_logic(self, mock_get_table, mock_auth):
        ...
        event = _make_event({
            ...
            "filters": [
                ...
                {"field": "status", "operator": "equals", "value": "needs_review"},
            ],
        })
        result = handler(event, None)
        body = json.loads(result["body"])

        # Only APP003 is digital + needs_review
        assert body["summary"]["total_applications"] == 1
...
class TestGrouping:
    ...
    def test_group_by_status(self, mock_get_table, mock_auth):
        ...
        assert groups["needs_review"]["count"] == 2
...
class TestExportHandler:
    ...
    def test_export_applies_filters(self, mock_get_table, mock_audit, mock_auth):
        ...
        # Header + 2 filtered rows (APP002 Smith + APP003 Lopez)
        assert len(lines) == 3
    ...
    def test_export_records_audit_log(self, mock_get_table, mock_audit, mock_auth):
        ...
        assert call_kwargs[1]["details"]["row_count"] == 2
