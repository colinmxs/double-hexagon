...
def _giveaway_year_from_body():
    """Extract giveaway_year from the JSON request body without consuming the stream."""
    try:
        body = request.get_data(as_text=True)
        if body:
            return json.loads(body).get("giveaway_year")
...
@app.route("/api/applications/<giveaway_year>/<application_id>", methods=["GET"])
@app.route("/api/applications/<application_id>", methods=["GET"])
def get_application_detail(application_id, giveaway_year=None):
    # Support giveaway_year as either path segment or query param
    if not giveaway_year:
        giveaway_year = request.args.get("giveaway_year")
    event = _build_event(
        path_params={"giveaway_year": giveaway_year, "application_id": application_id},
    )
    result = _get_handler("get_application_detail")(event, None)
    return _flask_response(result)


@app.route("/api/applications/<giveaway_year>/<application_id>", methods=["PUT"])
@app.route("/api/applications/<application_id>", methods=["PUT"])
def update_application(application_id, giveaway_year=None):
    if not giveaway_year:
        giveaway_year = request.args.get("giveaway_year")
    if not giveaway_year:
        giveaway_year = _giveaway_year_from_body()
    event = _build_event(
        path_params={"giveaway_year": giveaway_year, "application_id": application_id},
    )
...
def update_application_status(application_id):
    giveaway_year = request.args.get("giveaway_year") or _giveaway_year_from_body()
    event = _build_event(
        path_params={"giveaway_year": giveaway_year, "application_id": application_id},
    )
    result = _get_handler("update_application")(event, None)
...
def update_child_bike_number(application_id, child_id):
    giveaway_year = request.args.get("giveaway_year") or _giveaway_year_from_body()
    event = _build_event(
        path_params={"giveaway_year": giveaway_year, "application_id": application_id, "child_id": child_id},
    )
    result = _get_handler("update_application")(event, None)
    return _flask_response(result)
...
@app.route("/api/cost-dashboard/budget", methods=["GET", "PUT"])
def cost_budget():
    event = _build_event()
    result = _get_handler("get_cost_data")(event, None)
    return _flask_response(result)
