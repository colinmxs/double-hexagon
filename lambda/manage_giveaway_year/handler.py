...
- POST /giveaway-years/{year}/archive — Archive a year
...
def _handle_archive(year, user_context):
    ...
    for item in items:
        table.update_item(
            Key={"giveaway_year": year, "application_id": item["application_id"]},
            UpdateExpression="SET #ro = :ro",
            ExpressionAttributeNames={"#ro": "read_only"},
            ExpressionAttributeValues={":ro": True},
        )

    # Transition S3 objects to Glacier via storage class change
    s3 = _get_s3_client()
    bucket = _get_bucket_name()
    prefix = f"uploads/{year}/"
    try:
        ...
        logger.exception("Failed to transition S3 objects to Glacier")
...
def _handle_delete(event, year, user_context):
    ...
    items = resp.get("Items", [])
    for item in items:
        table.delete_item(Key={"giveaway_year": year, "application_id": item["application_id"]})
