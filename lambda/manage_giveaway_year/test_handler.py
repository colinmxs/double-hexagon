...
@patch("handler._get_applications_table")
@patch(AUTH_PATCH, return_value=_admin())
def test_archive_transitions_s3_to_glacier(mock_auth, mock_apps_table, mock_s3, mock_years, mock_write, mock_audit):
    ...
    resp = handler(_make_event("POST", path="/giveaway-years/2024/archive", path_params={"year": "2024"}), None)
    assert resp["statusCode"] == 200
    mock_s3.return_value.copy_object.assert_called_once()
    copy_call = mock_s3.return_value.copy_object.call_args
    assert copy_call[1]["StorageClass"] == "GLACIER"
...
def test_delete_removes_records(mock_auth, mock_apps_table, mock_s3, mock_active, mock_years, mock_write, mock_config, mock_audit):
    ...
    mock_apps_table.return_value.delete_item.assert_called_once()
