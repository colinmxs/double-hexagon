...
def handler(event, context, user_context):
    ...
    Path parameters:
        application_id: The application partition key (ULID).
    ...
    Requirements: 5.7, 5.8, 5.9, 5.10, 5.11, 9.2, 15.3
    """
    application_id = get_path_parameter(event, "id") or get_path_parameter(event, "application_id")

    if not application_id:
        return build_error_response(400, "Missing required path parameter: application_id")
    ...
    # Read giveaway_year from the fetched record (used for S3 version path)
