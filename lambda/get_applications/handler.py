...
def _get_first_drawing_key(application):
    """Get the raw S3 key for the first child's drawing image."""
    children = application.get("children", [])
    if not children:
        return None
    for child in children:
...
def _format_application(application):
    ...
    item = {
        ...
        "drawing_s3_key": _get_first_drawing_key(application),
    }
    return item


def _matches_search(application, search_term):
    """Check if an application matches the search term (reference number, family name, or agency name)."""
    search_lower = search_term.lower()

    # Search by reference number
    ref_num = application.get("reference_number", "")
    if ref_num and search_lower in ref_num.lower():
        return True
...
def _query_all(table, giveaway_year, page_size, exclusive_start_key=None):
    """Query all applications for a giveaway year using the year-index GSI."""
    from boto3.dynamodb.conditions import Key
